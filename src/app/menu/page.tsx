  ...
  return (
    <div className="min-h-screen bg-dark">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
            <div className="max-w-2xl">
              <span className="text-primary font-black text-xs uppercase tracking-[0.5em] mb-6 block">Premium Cravings</span>
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-6 overflow-x-auto no-scrollbar">
          ...
          {["All", ...categories].map((cat) => (
            <button
              ...
              className={`whitespace-nowrap px-8 py-3 rounded-2xl text-[12px] font-black uppercase tracking-widest transition-all ${
                activeCategory === cat
      ...
      <main className="max-w-7xl mx-auto px-6 py-24 pb-48">
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-40 bg-white/5 rounded-[4rem] border border-dashed border-white/10">
            ...
            <h3 className="text-2xl font-black uppercase italic text-gray-400">No cravings found.</h3>
            <p className="text-gray-600 mt-2">Try searching for something else or browse all categories.</p>
          </div>
        )}

        {/* Demo Footer Notice */}
        <div className="mt-40 glass-premium p-12 rounded-[3.5rem] border border-white/5 flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
          <div className="w-20 h-20 bg-accent/20 rounded-[2rem] flex items-center justify-center text-accent shrink-0">
            <Info className="w-10 h-10" />
          </div>
          <div>
            <h4 className="text-2xl font-black uppercase italic italic gold-text mb-2">Business Review Notice</h4>
      ...
      <AnimatePresence>
        {cartCount > 0 && !isCartOpen && (
          <motion.button
            ...
            className="fixed bottom-12 right-12 z-[45] w-20 h-20 bg-primary rounded-[2rem] flex items-center justify-center text-white shadow-[0_20px_50px_rgba(255,102,0,0.4)] hover:scale-110 active:scale-95 transition-all hidden md:flex group"
          >
            <ShoppingBag className="w-8 h-8 group-hover:rotate-12 transition-transform" />
            <span className="absolute -top-2 -right-2 w-8 h-8 bg-white text-primary rounded-xl flex items-center justify-center text-sm font-black shadow-lg">
              {cartCount}
            </span>
          </motion.button>
        )}
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 w-full px-6 md:hidden">
        <button
          onClick={() => setIsCartOpen(true)}
          className="w-full premium-gradient h-20 rounded-3xl flex items-center justify-between px-10 text-white font-black shadow-2xl shadow-primary/40 active:scale-95 transition-transform"
        >
          <div className="flex items-center gap-4">
            <ShoppingBag className="w-6 h-6" />
            <span className="uppercase tracking-widest text-sm">Cart ({cartCount})</span>
          </div>
          <span className="text-2xl italic">{formatPrice(cartTotal)}</span>
        </button>
      </div>
        ...
        {isCartOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-full max-w-xl bg-dark border-l border-white/5 z-[70] flex flex-col shadow-[-50px_0_100px_rgba(0,0,0,0.5)]"
            >
              <div className="p-10 border-b border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h2 className="text-3xl font-black italic gold-text leading-none uppercase">YOUR ORDER</h2>
              ...
              <div className="flex-grow overflow-y-auto p-10 space-y-10 custom-scrollbar">
                {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-30">
                    ...
                    <div>
                      <h4 className="text-2xl font-black uppercase italic mb-2">Cart is empty.</h4>
                    ...
                    <div key={item.id} className="flex gap-6 group items-center">
                      ...
                      <div className="flex-grow">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="font-black text-xl uppercase italic tracking-tighter">{item.name}</h4>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center bg-white/5 rounded-xl p-1.5 border border-white/10">
                            <button
                              ...
                              className="w-8 h-8 flex items-center justify-center hover:bg-primary rounded-lg transition-colors font-black"
                            >
                              -
                            </button>
                          ...
                          <span className="font-black text-xl text-primary">{formatPrice(item.price * item.quantity)}</span>
                        </div>
                      </div>
                    </div>
                  ))
                ...
                <div className="p-10 border-t border-white/5 bg-black/40 space-y-10">
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-end pt-6 border-t border-white/5">
                      ...
                      <span className="text-4xl font-black gold-text italic">{formatPrice(cartTotal)}</span>
                    </div>
                  </div>
                  
                  <Link
                    href="/order"
                    className="w-full premium-gradient py-8 rounded-[2rem] text-white font-black text-2xl text-center block shadow-2xl shadow-primary/30 hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest"
                  >
                    Checkout Now
                  </Link>
                </div>
